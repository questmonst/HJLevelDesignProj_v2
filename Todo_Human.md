...
- [ ] (나중에) **펀치 반동 덤블링 조건 추가** — 지금은 반동이 일어나면 항상 백덤블링 몽타주가 나옴. **공중 6m 이상일 때만** 재생하고, 낮은 높이에서는 생략하거나 짧은 반동 모션으로. AI에 요청 (C++: `BeginReboundMove()`에서 지면까지 거리 검사 후 몽타주 분기, 기준 높이는 DA 값으로)
